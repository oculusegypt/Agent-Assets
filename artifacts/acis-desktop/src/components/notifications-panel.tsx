...
import { Bell, X, CheckCheck, Zap, AlertTriangle, CheckCircle2, Activity, Cpu } from "lucide-react";
...
export function useNotifications() {
  ...
  const addNotification = useCallback((n: Omit<Notification, "id" | "time" | "read">) => {
    const notif: Notification = { ...n, id: crypto.randomUUID(), time: new Date(), read: false };
    setNotifications(prev => [notif, ...prev].slice(0, 50));
  }, []);

  useEffect(() => {
    wsSingleton.connect();
    return wsSingleton.addListener((data: WsEvent) => {
      ...
      switch (data.type) {
        case "agent_executed":
          title = `وكيل ${(data as any).agent_name || (data as any).agentId || "ذكاء اصطناعي"} نفّذ مهمة`;
          desc = (data as any).tokens_used ? `${(data as any).tokens_used} رمز · ${(data as any).model || ""}` : undefined;
          break;
        case "new_alert":
          title = (data as any).title || "تنبيه جديد";
          desc = (data as any).message || (data as any).description;
          break;
        case "job_completed":
          title = `✅ اكتملت: ${(data as any).data?.phase || "مهمة إنتاج"}`;
          desc = (data as any).data?.projectName;
          break;
        case "job_failed":
          title = `❌ فشلت: ${(data as any).data?.phase || "مهمة إنتاج"}`;
          desc = (data as any).data?.error;
          break;
        case "system_health":
          if (((data as any).score ?? 100) < 70) {
            title = `تحذير: صحة النظام ${(data as any).score}%`;
            desc = "يُنصح بالفحص الفوري";
          } else return;
          break;
        default:
          if (!data.type || data.type === "ping") return;
          title = (data as any).title || (data as any).message || `حدث: ${data.type}`;
          desc = (data as any).description;
      }
...
export function NotificationsPanel({ compact }: Props) {
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      {open && (
        <div className="absolute bottom-full mb-2 left-0 z-50 w-80 bg-card border border-border/60 rounded-xl shadow-2xl shadow-black/40 overflow-hidden" dir="rtl">
          <div className="flex items-center justify-between px-3 py-2.5 border-b border-border/50 bg-secondary/30">
            <div className="flex items-center gap-2">
              {notifications.length > 0 && (
                <button onClick={clearAll} className="text-[10px] font-mono text-muted-foreground/50 hover:text-red-400 transition-colors">مسح الكل</button>
              )}
              {unreadCount > 0 && (
                <button onClick={markAllRead} className="text-[10px] font-mono text-muted-foreground/50 hover:text-primary transition-colors flex items-center gap-1">
                  <CheckCheck size={10} /> تعليم كمقروء
                </button>
              )}
            </div>
            <div className="flex items-center gap-2">
            ...
            {notifications.length === 0 ? (
              ...
              <div className="divide-y divide-border/30">
                {notifications.map(n => {
                  ...
                  const Icon = s.icon;
                  return (
                    <div key={n.id} className={`flex items-start gap-2.5 px-3 py-2.5 hover:bg-secondary/30 transition-colors group ${!n.read ? "bg-primary/3" : ""}`}>
                      <button onClick={() => dismiss(n.id)} className="opacity-0 group-hover:opacity-100 transition-opacity p-0.5 hover:text-red-400 text-muted-foreground/30 shrink-0 mt-0.5">
                        <X size={10} />
                      </button>
                      <div className="flex-1 min-w-0 text-right">
                        <div className="flex items-start justify-between gap-1">
                          <span className="text-[10px] font-mono text-muted-foreground/40 shrink-0 mt-0.5">{relativeTime(n.time)}</span>
                          <p className={`text-xs font-medium leading-snug ${!n.read ? "text-foreground" : "text-foreground/70"}`}>{n.title}</p>
                        </div>
                        {n.desc && <p className="text-[10px] text-muted-foreground mt-0.5 line-clamp-2">{n.desc}</p>}
                      </div>
