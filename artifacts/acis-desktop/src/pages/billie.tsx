...
export default function BilliePage() {
  ...
  const [tab, setTab] = useState<"chat" | "overview" | "news" | "alerts" | "complaints" | "surgery" | "search">("chat");
  const [searchQuery, setSearchQuery] = useState("");
  const [searchLoading, setSearchLoading] = useState(false);
  const [searchResult, setSearchResult] = useState<{ query: string; results: any[]; summary: string; model: string } | null>(null);
  ...
  const TABS = [
    ...
    { key: "search",     label: "بحث الويب",               icon: Search },
  ] as const;
        ...
        <div className="space-y-4">
          <div className="rounded-xl border border-border/50 bg-card overflow-hidden">
            <div className="px-4 py-3 border-b border-border/30 bg-secondary/20 flex items-center gap-2">
              ...
              <span className="text-sm font-semibold">بحث الويب بمساعدة بيليه</span>
            </div>
                ...
                <input
                  ...
                  onKeyDown={e => {
                    if (e.key === "Enter" && !searchLoading) {
                      ...
                      fetch(`${BASE}/api/billie/web-search`, {
                        method: "POST", headers: { "Content-Type": "application/json" },
                        body: JSON.stringify({ query: searchQuery }),
                      }).then(r => r.json()).then(d => setSearchResult(d)).catch(() => {}).finally(() => setSearchLoading(false));
                    }
                  }}
                  placeholder="ابحث عن أي موضوع... (اضغط Enter)"
                  className="flex-1 bg-secondary/50 border border-border/40 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-primary/50 transition-colors"
                  dir="rtl"
                />
                <button
                  onClick={() => {
                  ...
                  className="px-4 py-2 rounded-lg bg-primary/20 hover:bg-primary/30 border border-primary/40 text-primary text-sm font-medium transition-all disabled:opacity-40"
                >
                ...
                <div className="space-y-3">
                  ...
                  <div className="rounded-lg border border-primary/20 bg-primary/5 p-3">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs font-mono text-primary">ملخص بيليه</span>
                      {searchResult.model && <span className="text-[10px] font-mono text-muted-foreground/50">· {searchResult.model}</span>}
                    </div>
                    <p className="text-sm leading-relaxed text-right">{searchResult.summary}</p>
                  </div>

                  {/* Raw Results */}
                  {searchResult.results.length > 0 && (
                    <div className="space-y-2">
                      <div className="text-xs font-mono text-muted-foreground uppercase tracking-widest">نتائج البحث ({searchResult.results.length})</div>
                      {searchResult.results.map((r: any, i: number) => (
                        <div key={i} className="rounded border border-border/40 bg-secondary/20 p-3 text-right hover:border-border/70 transition-colors">
                          <div className="flex items-start justify-between gap-2">
                            <a href={r.url} target="_blank" rel="noopener noreferrer"
                              className="text-[10px] font-mono text-muted-foreground/50 hover:text-primary transition-colors flex items-center gap-1 shrink-0">
                              <ExternalLink size={9} />
                            </a>
